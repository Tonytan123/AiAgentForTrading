...
class MomentumAgent(BaseFeatherlessAgent):
    ...
    def heuristic_evaluate(self, ticker: str, context_data: dict) -> AgentEvaluation:
        ...
        if price > sma_20:
            score += 0.25
            reasons.append(f"Above SMA20 (${sma_20:.1f})" if is_en else f"站上SMA20 (${sma_20:.1f})")
        if chg_pct > 0:
            score += min(0.20, chg_pct * 0.05)
            reasons.append(f"Day +{chg_pct:.1f}%" if is_en else f"日内+{chg_pct:.1f}%")
        elif chg_pct < -3.0:
            score -= 0.15
        if 40 <= rsi <= 65:
            score += 0.15
            reasons.append(f"RSI={rsi:.0f} Healthy Bull" if is_en else f"RSI={rsi:.0f}健康多头")
        elif rsi > 75:
            score -= 0.20
        if vol_surge >= 1.3:
            score += 0.15
            reasons.append(f"Vol {vol_surge:.1f}x surge" if is_en else f"放量{vol_surge:.1f}x")

        rat = " | ".join(reasons) if reasons else ("Momentum stable" if is_en else "动量平稳")
        return AgentEvaluation(agent_name=self.name, score=min(1.0, max(0.0, score)), rationale=rat)
